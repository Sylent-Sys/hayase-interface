...
import native, { startTorrentInfoStream } from "../native";
...
export const server = new (class ServerClient {
  last = persisted<{ media: Media; id: string; episode: number } | null>(
    "last-torrent",
    null,
  );
  active =
  ...
  stats = this._streamFirstStatsStore(defaultTorrentInfo);
  ...
  _timedSafeStore<T>(
    ...
    duration = SUPPORTS.isUnderPowered ? 15000 : 5000,
  ) {
  ...
  _streamFirstStatsStore(
    defaultData: TorrentInfo,
    duration = SUPPORTS.isUnderPowered ? 3000 : 200,
  ) {
    return writable<TorrentInfo>(defaultData, (set) => {
      let pollingTimer = 0;
      ...
      const pollStatus = async () => {
        if (isDestroyed || !currentHash) return;
        try {
          set(await native.torrentInfo(currentHash));
        } catch (error) {
          console.error(error);
        }
        pollingTimer = setTimeout(pollStatus, duration);
      };

      const startPolling = () => {
        if (isDestroyed || !currentHash || pollingTimer) return;
        pollStatus();
      };
      ...
      const startStreamForHash = (hash: string) => {
        stopAllStreams();
        stopPolling();
        stopStream = startTorrentInfoStream(hash, {
          onStatus: (info) => {
            if (currentHash !== hash || isDestroyed) return;
            stopPolling();
            set(info);
          },
          onFallback: () => {
          ...
          onError: (error) => {
            console.error("[torrent status stream] error", error);
          },
        });
      };

      const unsubscribeActive = this.active.subscribe(
        async (
          activePromise:
            | Promise<{
                media: Media;
          ...
          try {
            ...
            if (version !== activeVersion || isDestroyed) return;
            ...
            set({ ...defaultData, hash: nextHash });
            startStreamForHash(nextHash);
          } catch (error) {
            if (version !== activeVersion || isDestroyed) return;
            console.error(error);
            startPolling();
  ...
  constructor() {
    ...
    if (last) {
      ...
      debug(
        "restored last torrent",
        last.id,
        last.media.title?.userPreferred,
        last.episode,
      );
    }
  ...
  async updateLibrary() {
    ...
    this.downloaded.value = library.then(
      (lib) => new Set(lib.map(({ hash }) => hash)),
    );
  ...
  play(
    ...
    media: Media,
    episode: number,
    torrent: string | ArrayBufferView = id,
  ) {
    if (!media || !id) return;
  ...
  async playFile(id: ArrayBufferView, media: Media, episode: number) {
    ...
    const { infoHash } = await (parseTorrent(id) as Promise<{
      infoHash: string;
    }>);
  ...
  async _play(
    id: string,
    torrent: string | ArrayBufferView,
    media: Media,
    episode: number,
  ) {
    const result = {
      ...
      media,
      episode,
      files: await native.playTorrent(torrent, media.id, episode),
    };
