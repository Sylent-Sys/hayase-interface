 ...
 */
async function extractInfoHash(
  torrent: string | ArrayBufferView,
): Promise<string> {
  if (typeof torrent === "string") {
    const magnetMatch = torrent.match(
      /urn:btih:([a-fA-F0-9]{40}|[A-Z2-7]{32})/i,
    );
  ...
  const parsed = await (
    parseTorrent as unknown as (
      buf: Uint8Array,
    ) => Promise<{ infoHash: string }>
  )(torrent as unknown as Uint8Array);
...
function makeAuth<T>(
  popup: Window | null,
  callback: (data: { hash: string; search: string }) => T | undefined,
) {
  return new Promise<T>((resolve, reject) => {
    ...
    const timeout = setTimeout(
      () => destroy(new Error("Authentication timed out")),
      5 * 60 * 1000,
    ); // 5 minutes
...
export default Object.assign<Native, Partial<Native>>(
  {
    authAL: (url: string) => {
      return makeAuth(
        open(
          url,
          "authframe",
          SUPPORTS.isAndroid ? "popup" : "popup,width=382,height=582",
        ),
        ({ hash }) => {
          if (hash.startsWith("#access_token=")) {
            return Object.fromEntries(
              new URLSearchParams(hash.replace("#", "?")).entries(),
    ...
    checkAvailableSpace: () =>
      new Promise((resolve) =>
        setTimeout(
          () => resolve(Math.floor(Math.random() * (1e10 - 1e8 + 1) + 1e8)),
          1000,
        ),
      ),
    ...
    isApp: false,
    playTorrent: (torrent: string | ArrayBufferView) =>
      fetchTorrentFiles(torrent),
    rescanTorrents: async () => undefined,
    deleteTorrents: async (hashes) => {
      await Promise.all(
        hashes.map(async (hash) => {
          const res = await fetch(`/api/torrent/${encodeURIComponent(hash)}`, {
            method: "DELETE",
          });
          if (!res.ok && res.status !== 404) {
            const body = await res.text();
            throw new Error(
              `Failed to delete torrent ${hash}: ${res.status} ${body}`,
            );
          }
        }),
      );
    ...
    getDisplays: async (cb) =>
      cb([{ friendlyName: "Display 1", host: "display1" }]),
